...
export default function Home() {
  ...
  const cityOptions = useMemo(() => {
    ...
    return Array.from(cities).map((location) => ({ value: location, label: location }))
  }, [flight,,hotel])
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div className="col-span-1">
              <SearchInput
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setTravellers(parseInt(e.target.value) || 1)
                }
...
function NavItem({ icon, text, active = false, onClick }: any) {
  return (
    <button className={`flex flex-col items-center p-2 rounded-lg transition-colors ${active ? "text-blue-500" : "text-gray-600 hover:text-blue-500"}`} onClick={onClick}>
      {icon}
      <span>{text}</span>
    </button>
  )
